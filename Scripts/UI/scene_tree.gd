...
@onready var tab_container: VBoxContainer = $ScrollContainer/SceneTabContainer
...
var _drag_threshold: float = 5.0 # Pixels to move before it counts as a panel drag
...
func _ready() -> void:
	...
	dock_btn.flip_h = false # Default docked state visual
	_update_layout()

func _input(event: InputEvent) -> void:
	if not visible: return

	# --- Smart Dragging (Y-axis only, anywhere on panel) ---
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			# Check if mouse is inside the panel bounds
		...
		if _potential_drag and not _is_dragging:
			# If mouse moves more than the threshold, start dragging the panel!
			if abs(event.global_position.y - _drag_start_pos.y) > _drag_threshold:
				_is_dragging = true
				_potential_drag = false
				scroll_container.release_focus() # Prevent accidental scrolling while dragging
		
		if _is_dragging:
			var delta_y = event.global_position.y - _drag_start_pos.y
			global_position.y = _drag_panel_start_y + delta_y
			global_position.x = _initial_x # Lock X axis
			get_viewport().set_input_as_handled()

	# --- Resizing (Right, Top-Right, Bottom-Right only) ---
	if _is_resizing:
...
func _connect_signals() -> void:
	...
	# No longer connecting bg.gui_input for dragging!
...
func _update_layout() -> void:
	...
	var w: float = size.x
	var h: float = size.y
	var m: float = 16.0 # 9-slice margin
	
	# Scroll Container (Padded inside 9-slice)
	scroll_container.position = Vector2(m, m)
	scroll_container.size = Vector2(w - m * 2.0, h - m * 2.0)
	
	# Dock Button (Top Right)
	dock_btn.position = Vector2(w - dock_btn_width - 8.0 * (-0.3), 15.0)
...
func _on_handle_input(event: InputEvent, handle: Control) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
		...
		_resize_dir = Vector2i(
			(1 if "E" in n else 0), # Only East (Right) directions allowed
...
func _apply_resize(mouse_pos: Vector2) -> void:
	...
	var n_right  := _resize_rect_orig.end.x + delta.x # Only right edge moves on X
	...
	# Apply (Lock left edge completely!)
	offset_left = n_left
...
# ────────────────────── Tabs ──────────────────────
func add_scene_tab(node_name: String, icon: Texture2D = null) -> void:
	var tab_instance := SCENE_TAB_SCENE.instantiate()
	tab_container.add_child(tab_instance)
	
	if icon:
		tab_instance.set_icon(icon)
